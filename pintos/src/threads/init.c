...
static void run_func(void *aux) {
  ...
  if (fname[start_index] == 't' && fname[start_index + 1] == 'w'
    ...
    tid_t wthread = thread_create("Thread_Stats", PRI_MAX, &running_thread_stat,
        NULL);
    setPriority(fname, 10);
    if (is_run) {
      thread_wait(wthread);
  ...
  } else if (fname[start_index] == 'b' && fname[start_index + 1] == 'g'
      && fname[start_index + 2] == 'g' && fname[start_index + 3] == 'd') {
    tid_t wthread = thread_create("Background", PRI_MAX, &dummy_thread,
        NULL);
    setPriority(fname, 10);
    if (is_run) {
      thread_wait(wthread);
...
static void cook_pizza_test(void *aux) {
  ...
  NULL);
  tid_t cthread = thread_create("BREAD_LOW", PRI_MIN, &prepare_bread_test,
      NULL);
