...
extern void save_and_switch_context(
    struct interrupts_stack_frame *cur_stack_frame,
    struct interrupts_stack_frame *next_stack_frame, struct thread *cur,
    struct thread *next);
...
void set_current_interrupts_stack_frame(
    struct interrupts_stack_frame *stack_frame);
...
static struct thread* thread_get_next_priority_thread_to_run(void);
static void thread_save_stack_frame(struct thread* thread,
    struct interrupts_stack_frame* stack_frame);
...
static void context_switch(struct thread* next_thread,
    struct interrupts_stack_frame* stack_frame);
 ...
 */
tid_t thread_create(const char *name, int32_t priority, thread_func *function,
    void *aux_parameter) {
  ASSERT(PRI_MIN <= priority && priority <= PRI_MAX);
  ASSERT(name != NULL);
...
void thread_exit(void) {
  ...
  //Start thread_wait
  while (!list_empty(&thread_current()->wait_list)) {
    list_push_back(&ready_list, list_pop_front(&thread_current()->wait_list));
  }
  //End thread_wait
...
static struct thread* thread_get_running_thread(void) {
  ...
  if (interrupts_was_irq_generated()) {
    ...
    struct interrupts_stack_frame *stack_frame =
        get_current_interrupts_stack_frame();
...
bool my_order_function(const struct list_elem *a, const struct list_elem *b,
    void *aux) {
...
static void context_switch(struct thread* next,
    struct interrupts_stack_frame* stack_frame) {
  /* Note: If the string.h library is not supported, the next assignment can't be done because the
   * GNU GCC compiler uses memcpy() function to copy the data. */
...
void show_running_thread_status() {
  ...
  for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
    ...
    printf("%20s \t\t\t %3d \t\t\t ", t->name, t->running_ticks);
    printf("%s\t\t  \n", status);
  }
  printf("----------------------------------------------------------------------------------\n");
